<template>
  <Message>
    ...
    <Message-Body>
      <Message-Item :data="this.articles" :isLoading="this.isLoading" moreURL="https://abcnews.go.com">
...
export default {
  ...
  methods: {
    timeAgo,
    async request() {
      const response = await fetch('https://api.tab.ludicrous.xyz/v1/abc-news/index');
  ...
  created() {
    const storage = getData('abc-news');
    if (storage) {
