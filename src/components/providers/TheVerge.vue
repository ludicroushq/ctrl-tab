<template>
  <Message>
    <Message-Header background="#e5127d" :isLoading="this.isFetching" :edit="this.edit" :remove="remove" :name="this.name">
      The Verge
    </Message-Header>
    <Message-Body>
      <Message-Item :data="this.articles" :isLoading="this.isLoading" moreURL="https://www.theverge.com">
...
export default {
  ...
  methods: {
    timeAgo,
    async request() {
      const response = await fetch('https://api.tab.ludicrous.xyz/v1/the-verge/index');
  ...
  created() {
    const storage = getData('the-verge');
    if (storage) {
