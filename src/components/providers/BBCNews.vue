<template>
  <Message>
    <Message-Header background="#bb1919" :isLoading="this.isFetching" :edit="this.edit" :remove="remove" :name="this.name">
      BBC News
    </Message-Header>
    <Message-Body>
      <Message-Item :data="this.articles" :isLoading="this.isLoading" moreURL="https://www.bbc.com/news">
...
export default {
  ...
  methods: {
    timeAgo,
    async request() {
      const response = await fetch('https://api.tab.ludicrous.xyz/v1/bbc-news/index');
  ...
  created() {
    const storage = getData('bbc-news');
    if (storage) {
