<template>
  <Message>
    <Message-Header background="#00aeef" :isLoading="this.isFetching" :edit="this.edit" :remove="remove" :name="this.name">
      Mashable
    </Message-Header>
    <Message-Body>
      <Message-Item :data="this.articles" :isLoading="this.isLoading" moreURL="https://mashable.com">
...
export default {
  ...
  methods: {
    timeAgo,
    async request() {
      const response = await fetch('https://api.tab.ludicrous.xyz/v1/mashable/index');
  ...
  created() {
    const storage = getData('mashable');
    if (storage) {
