<template>
  <Message>
    <Message-Header background="#00a562" :isLoading="this.isFetching" :edit="this.edit" :remove="remove" :name="this.name">
      TechCrunch
    </Message-Header>
    <Message-Body>
      <Message-Item :data="this.articles" :isLoading="this.isLoading" moreURL="https://techcrunch.com">
...
export default {
  ...
  methods: {
    timeAgo,
    async request() {
      const response = await fetch('https://api.tab.ludicrous.xyz/v1/techcrunch/index');
  ...
  created() {
    const storage = getData('techcrunch');
    if (storage) {
