<template>
  <Message>
    <Message-Header background="#be2819" :isLoading="this.isFetching" :edit="this.edit" :remove="remove" :name="this.name">
      The Times Of India
    </Message-Header>
    <Message-Body>
      <Message-Item :data="this.articles" :isLoading="this.isLoading" moreURL="https://timesofindia.indiatimes.com">
...
export default {
  ...
  methods: {
    timeAgo,
    async request() {
      const response = await fetch('https://api.tab.ludicrous.xyz/v1/the-times-of-india/index');
  ...
  created() {
    const storage = getData('the-times-of-india');
    if (storage) {
